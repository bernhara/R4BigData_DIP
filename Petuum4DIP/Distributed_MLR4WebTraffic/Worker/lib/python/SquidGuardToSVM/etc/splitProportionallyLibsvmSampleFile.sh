...
: ${splitter:="${HERE}/splitEqualyLibsvmSampleFile.sh"}
...
Usage ()
{
    ...
    echo "${msg}" 1>&2
    echo "Usage: <cmd> <Libsvm input format compliant file (csv)> <generated samples output prefix path> <slice size> [<slice size>...] " 1>&2
...
"${splitter}" "${libsvmSrcFile}" $nb_quantums "${tmp_dir}/quantums."
