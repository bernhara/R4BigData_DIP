...
def normalize_url(url, charset='utf-8'):
    ...
    if url[0] not in ['/', '-'] and ':' not in url[:7]:
        # find the port
        url_with_standard_scheme = 'http://' + url
        parsed_url = urlsplit(url_with_standard_scheme)
        port = parsed_url.port
        if not port:
            # no scheme, no port => http:
            url = 'http://' + url
        else:
            schemes = [scheme for scheme, default_port in DEFAULT_PORT.items() if port == default_port]
            if len(schemes) == 1:
                # we got a known port in the
                url = schemes[0] + '://' + url
            else:
                # the port is not well known. Assume 'http'
                url = 'http://' + url
